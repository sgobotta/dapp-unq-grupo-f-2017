...
export class PurchaseMail extends Mail {
...
export class PurchaseMailBuilder {
  ...
  private getDeliveryText() {
    const text = Translations.en.purchase.deliveryText(
  ...
  private getPickUpText() {
    const text = Translations.en.purchase.pickUpText(
      this.menuName, this.quantity, this.menuDescription, this.price,
      this.deliveryPrice, this.finalPrice, this.deliveryTime
    )
    return text;
  ...
  public build() {
    ...
    const mail = new PurchaseMail(this.consignee, text);
    this.clear();
    return mail;
  }
}